...
class BookModel:
    ...
    def read_book_by_id(self, book_id: int) -> dict:
        try:
    ...
    def update_book(self, book_id: int, titulo: str, autor: str, ano: int, preco: float) -> int:
        try:
            result = self.collection.update_one({"_id": ObjectId(book_id)}, {"$set": {"titulo": titulo, "autor": autor, "ano": ano, "preco": preco}})
            if result.modified_count:
                print(f"Book {book_id} updated with Titulo => {titulo}, Autor => {autor}, Ano => {ano} and Preco => {preco}")
            else:
                print(f"No book found with id {book_id}")
